...
public class BasicValidator {
	...
	public static boolean isCharAllowed(String checkString) throws InvalidInputException {
		...
		while ((i <= checkString.length() - 1) && (checkString.length() >= 3)) {
			if ((checkString.charAt(i) >= 'A' && checkString.charAt(i) <= 'Z')
					...
					|| (checkString.charAt(i) >= '0' && checkString.charAt(i) <= '9') || checkString.charAt(i) == ' ') {
				isValid = true;
	...
	/**
	 * Checks weather the given password meets the given level constrains Password
	 * should contain a Capital letter,small letter ,Special Character and a number.
	 * 
	 * @param password
	 * @throws InvalidInputException
	 */
