 ...
 * errorMessage; String category = request.getParameter("category"); String type
 * = request.getParameter("type"); try { //boolean isAdded =
 * FlowerManager.deleteFlower(category, type); if (isAdded) { errorMessage =
 * "Successfully Deleted Flower"; } else { errorMessage =
 * "Unable to add flower type"; } } catch (Exception e) { e.printStackTrace();
